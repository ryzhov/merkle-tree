...
describe('Trie', () => {
    describe('with a single word', () => {
        ...
        it('should connect the root to the first letter', () => {
            const firstNode = trie.root.getChild('h');
            assert.equal(firstNode.key, 'h', 'expected the `key` of the first node to be `h`');
            assert(firstNode.getChild('e'), 'expected the `children` of the first node to have a connection to the next letter');
        ...
        it('should connect the root to the second letter', () => {
            const firstNode = trie.root.getChild('h');
            const secondNode = firstNode.getChild('e');
            assert.equal(secondNode.key, 'e', 'expected the `key` of the first node to be `e`');
            assert(secondNode.getChild('y'), 'expected the `children` of the second node to have a connection to the next letter');
        ...
        it('should connect the root to the third letter', () => {
            const firstNode = trie.root.getChild('h');
            const secondNode = firstNode.getChild('e');
            const thirdNode = secondNode.getChild('y');
            assert.equal(thirdNode.key, 'y', 'expected the `key` of the first node to be `y`');
            assert.equal(thirdNode.children.length, 0, 'expected to have no `children` for the final node');
        ...
        words.forEach((word) => {
            describe(`for ${word}`, () => {
                it('should connect to the final letter', () => {
                    const finalNode = word.split("").reduce((node, letter) => node.getChild(letter), trie.root);
                    assert(finalNode);
